...
## Confirmed against the official docs, 2026-08-30

This spike's findings were re-checked against `docs.midnight.network` after the fact. Everything above holds, and three points are now settled with citations rather than inference:

- **NIGHT genuinely moves between shielded and unshielded** — *"Both NIGHT and custom tokens can move between shielded and unshielded state"* ([tokens/overview](https://docs.midnight.network/tokens/overview)). The `{ raw, tag }` shape of `UnshieldedTokenType`/`ShieldedTokenType` in the ledger API is the mechanism this spike reverse-engineered from testing: shielded-ness belongs to the coin, not to the token.
- **DUST is not a payment token at all** — *"You cannot transfer DUST between users"* ([DUST architecture](https://docs.midnight.network/concepts/dust-architecture)). It is a non-transferable fee resource generated by NIGHT holdings. A salary denominated in DUST is a factual error, which settles the DUST-vs-NIGHT labelling question in the apps.
- **Why the money must bypass the contract.** Compact's `sendShielded` does not currently create coin ciphertexts, so a contract sending shielded value to anyone other than its own caller leaves that person with **no notification the coin exists** ([shielded token guide](https://docs.midnight.network/tokens/shielded-token)). Wallet-to-wallet is not a shortcut around a hard problem — it is the documented way to pay a third party today.

`payment.ts` was checked line by line against the connector's own type signatures and matches exactly, including its honest note that `submitTransaction` resolves to `void` and so yields no txId.

## What this means for the project

The plan holds as originally designed. No fallback to unshielded payments needed.

**One gotcha for whoever wires the demo**, restating the lesson above because it will cost an hour otherwise: a wallet needs an existing *shielded* balance to send shielded value. The funding CLI hands out unshielded NIGHT only, so an employer wallet funded that way cannot pay anyone until it holds shielded funds.
